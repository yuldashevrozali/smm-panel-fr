...
function DashboardApp() {
  ...
  useEffect(() => {
    if (!token) return;
    api.services(token).then((data) => { setServices(data); setSelectedService(null); setQuantity(0); })
  ...
  const selectedPlatformServices = useMemo(() => {
    ...
    return services.filter((service) => service.category === selectedPlatform);
  ...
  return <div className="saas-shell">
    ...
    <main className="app-main"><header className="app-topbar"><div className="app-topbar__start"><button type="button" className="mobile-nav-toggle" aria-label="Open dashboard menu" onClick={() => setSidebarOpen(true)}><span /><span /><span /></button><div><p className="section-kicker">{heading[view][0]}</p><h1>{heading[view][1]}</h1></div></div><div className="app-topbar__end"><div className="account-chip"><span>{(user?.first_name?.[0] ?? "U").toUpperCase()}</span><div><strong>{user?.first_name ?? t.dashboard.accountChip}</strong><small>{user?.username ? `@${user.username}` : t.dashboard.telegramUser}</small></div></div><label className="language-switcher language-switcher--inline"><span className="sr-only">{t.dashboard.selectLanguage}</span><select value={locale} onChange={(event) => setLocale(event.target.value as "en" | "uz" | "ru")}>{locales.map((item) => <option key={item.code} value={item.code}>{item.label}</option>)}</select></label></div></header>
      ...
      {view === "new-order" && <section className="order-layout"><form className="content-card order-form" onSubmit={placeOrder}><div className="card-heading"><div><p>{t.dashboard.orderDetails}</p><h2>{t.dashboard.setUpOrder}</h2></div></div>{servicesPending ? <IntegrationPending title={t.dashboard.servicesIntegration} description={t.dashboard.servicesIntegrationText} /> : loadingServices ? <div className="skeleton large" /> : <><div className="order-field-stack"><label>{t.servicesPage.platformLabel}<select value={selectedPlatform} onChange={(event) => handlePlatformChange(event.target.value)}><option value="" disabled>Select a platform</option>{categories.map((category) => <option key={category} value={category}>{category}</option>)}</select></label><label>{t.dashboard.serviceSelect}<select value={selectedService?.service ?? ""} onChange={(event) => { const service = selectedPlatformServices.find((item) => String(item.service) === event.target.value) ?? null; setSelectedService(service); setQuantity(service?.min ?? 0); setOrderRequestKey(null); }} disabled={!selectedPlatform}><option value="" disabled>{selectedPlatform ? t.dashboard.selectService : "Choose a platform first"}</option>{selectedPlatformServices.map((service) => <option key={service.service} value={service.service}>{service.name}</option>)}</select></label></div>{selectedPlatform && !selectedPlatformServices.length && <div className="empty-state order-empty-state"><span>◈</span><h3>{t.dashboard.noServicesAvailable}</h3><p>{t.dashboard.checkBackLater}</p></div>}{selectedService && <div className="service-detail"><strong>{selectedService.name}</strong><p>{selectedService.description || t.dashboard.serviceDetails}</p><span>{t.dashboard.minimum} {selectedService.min.toLocaleString()} · {t.dashboard.maximum} {selectedService.max.toLocaleString()}</span></div>}<label>{t.dashboard.targetLink}<input type="url" value={link} onChange={(event) => { setLink(event.target.value); setOrderRequestKey(null); }} placeholder={t.dashboard.insertLink} required /></label><label>{t.dashboard.quantity}<input type="number" value={quantity || ""} onChange={(event) => { setQuantity(Number(event.target.value)); setOrderRequestKey(null); }} min={selectedService?.min} max={selectedService?.max} required /></label><button className="primary-btn full" disabled={submitting || !selectedService}>{submitting ? t.dashboard.submitting : t.dashboard.submitOrder}</button></>}</form><aside className="summary-card"><p>{t.dashboard.orderSummary}</p><h2>{selectedService ? selectedService.name : selectedPlatform || t.dashboard.chooseService}</h2><div><span>{t.dashboard.rate}</span><strong>{selectedService ? `${money(selectedService.rate)} / 1K` : "—"}</strong></div><div><span>{t.dashboard.estimatedPrice}</span><strong>{selectedService ? `${money(estimated)}` : "—"}</strong></div><small>{t.dashboard.estimateOnly}</small></aside></section>}
