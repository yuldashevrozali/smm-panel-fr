...
function DashboardApp() {
  ...
  return <div className="saas-shell">
    <aside className="app-sidebar"><div className="app-brand"><span className="brand-mark small-mark">S</span>SMMLY</div><nav>{navigation.map((item) => <button key={item.id} className={view === item.id ? "side-link active" : "side-link"} onClick={() => { setView(item.id); setError(null); }}><span>{item.icon}</span>{item.label}</button>)}</nav><button className="logout-link" onClick={logout}>{t.dashboard.signOut} <span>→</span></button></aside>
    <main className="app-main"><header className="app-topbar"><div><p className="section-kicker">{heading[view][0]}</p><h1>{heading[view][1]}</h1></div><div className="account-chip"><span>{(user?.first_name?.[0] ?? "U").toUpperCase()}</span><div><strong>{user?.first_name ?? t.dashboard.accountChip}</strong><small>{user?.username ? `@${user.username}` : t.dashboard.telegramUser}</small></div></div><label className="language-switcher language-switcher--inline"><span className="sr-only">{t.dashboard.selectLanguage}</span><select value={locale} onChange={(event) => setLocale(event.target.value as "en" | "uz" | "ru")}>{locales.map((item) => <option key={item.code} value={item.code}>{item.label}</option>)}</select></label></header>
